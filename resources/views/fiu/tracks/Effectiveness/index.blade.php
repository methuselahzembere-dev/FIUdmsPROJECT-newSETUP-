    ...
    >
        <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div class="space-y-3">
               ...
               {{-- 🌟 FIXED: Iterates through the plural collection, producing unique identity cards for each row item --}}
@foreach($immediateOutcomes as $io)
    <div class="mb-6 p-6 bg-white border border-slate-100 rounded-3xl shadow-sm">
        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-violet-600">Effectiveness / Immediate Outcome Node</p>
        ...
        <p class="mt-2 max-w-4xl text-sm leading-6 text-slate-600">
            {{ $io->description ?: 'Browse this Immediate Outcome through a split dashboard. Keep sibling sub-IOs visible in the left panel while reviewing documents in the main workspace.' }}
        </p>
    </div>
@endforeach
            </div>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="mx-auto max-w-xl space-y-3">
                                                ...
                                                <p class="text-sm text-slate-500">Choose another sub-IO from the sidebar or add the first Effectiveness document directly to {{ $selectedSubOutcome?->code ?? $immediateOutcome->code }}.</p>
                                                <div>
                                                    <a
                                                        href="{{ route('fiu.effectiveness.folders.documents.create', ['code' => $immediateOutcome->code, 'sub_io' => $selectedSubOutcome?->code]) }}"
